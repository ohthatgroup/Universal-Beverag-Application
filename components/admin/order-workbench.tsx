...
function resolveReturnTo(value: string | undefined) {
  ...
  if (trimmed.startsWith('/admin/orders/')) return '/admin'
  return trimmed
}
...
interface OrderWorkbenchProps {
  orderId: string
  /** Optional `?returnTo=` deep-link target. Used by the standalone
   *  page; the side-pane consumer can omit it. */
  returnTo?: string
}

/**
 * Order detail body — rendered both by the standalone page
 * (`/admin/orders/[id]`) and by the desktop directory side-pane.
 *
 * Wraps `<AdminOrderEditor>` with the data fetch + server actions
 * (mark-delivered, cancel, delete). The editor itself handles its
 * own layout chrome.
 */
export async function OrderWorkbench({
  orderId,
  returnTo: returnToProp,
}: OrderWorkbenchProps) {
  const id = orderId
  const returnTo = resolveReturnTo(returnToProp)
  ...
  async function markDelivered() {
    ...
    await actionDb.query(
      `update orders
       ...
       where id = $1`,
      [orderIdLocal, now, submittedAt ?? now],
    )
    redirect(`/admin/orders/${orderIdLocal}`)
  }
  ...
  async function deleteOrder() {
    ...
    await actionDb.transaction(async (client) => {
      await client.query('delete from order_items where order_id = $1', [orderIdLocal])
      await client.query('delete from orders where id = $1', [orderIdLocal])
    })
