...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (payload.action === 'reorder') {
      ...
      await db.transaction(async (client) => {
        for (let index = 0; index < orderedIds.length; index += 1) {
          await client.query('update brands set sort_order = $2 where id = $1', [orderedIds[index], index])
        }
      })
    ...
    try {
      await db.transaction(async (client) => {
        await client.query('update products set brand_id = null where brand_id = any($1::uuid[])', [ids])
        await client.query('delete from brands where id = any($1::uuid[])', [ids])
      })
    } catch (error) {
      if (
        typeof error === 'object' &&
