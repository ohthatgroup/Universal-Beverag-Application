...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (payload.action === 'reorder') {
      ...
      await db.transaction(async (client) => {
        for (let index = 0; index < orderedIds.length; index += 1) {
          await client.query(
            'update products set sort_order = $2 where id = $1 and customer_id is null',
    ...
    try {
      await db.query('delete from products where customer_id is null and id = any($1::uuid[])', [ids])
    } catch (error) {
      if (
        typeof error === 'object' &&
        error !== null &&
        ...
        throw new RouteError(
          ...
          'Some products are still referenced by orders or deals and cannot be deleted'
        )
      }
      throw error
    }
